...
/**
 * SloshGauge — React Bits (MIT), vendored for the phone's Control Center.
 *
 * Adapted in two places: the liquid's lean and the marker use the tank's
 * rendered size rather than the `width`/`height` props, because the phone
 * sizes the tank in its own points through CSS; and the empty catch blocks
 * carry a comment so the project's lint accepts them.
 */
...
export default function SloshGauge({
  ...
  const paint = () => {
    ...
    if (marker.current) marker.current.style.transform = `translateY(${((100 - L) * h) / 100}px)`;
  };
